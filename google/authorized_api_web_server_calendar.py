...
class RequestHandler(BaseHTTPRequestHandler):
  ...
  # arguments.
  flow = OAuth2WebServerFlow('516170101494-d9pv4e5eio34i7nlv0j18s2t5q0k9j90.apps.googleusercontent.com', #psys.argv[1],
                             'X5kX8F44Wq78ugd4S7ymVwUw', #sys.argv[2],
  ...
  def get_fake_user_from_url_param(self):
    ...
    fake_user = parse.parse_qs(parsed.query)['fake_user'][0]
  ...
  def get_fake_user_from_cookie(self):
    ...
    local_cookies = cookies.SimpleCookie()
    local_cookies.load(self.headers.get('Cookie'))
    fake_user = local_cookies['fake_user'].value
